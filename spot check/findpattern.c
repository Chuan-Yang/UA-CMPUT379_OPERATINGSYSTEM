...
int checkPattern(int patlength, unsigned char *pattern){
...
unsigned int findpattern (unsigned char *pattern, unsigned int patlength, struct patmatch *locations, unsigned int loclength){
    ...
    sysinfo(&s_info);// get total memory
    unsigned char* max_addr=(unsigned char*)s_info.totalram;
    for (current_point = 0; current_point <= max_addr; current_point++){

         //printf("%x\n",current_point);
         sigsetjmp (env, 1);
        if (current_point >= check_loop && current_point<=max_addr) {check_loop = current_point;}
        else    {break;}
        ...
        if (current_byte == pattern [0]){
            if ( checkPattern(patlength, pattern) == 1){
                ...
                if (loclength >= match_num) {
                    ...
                    current_byte = *current_point;
                    *current_point = current_byte;
